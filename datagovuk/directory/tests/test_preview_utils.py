...
def test_fetch_raw_content_returns_truncated_and_normalised_content(mock_get):
...
@patch("datagovuk.directory.preview_utils.requests.get")
def test_fetch_raw_content_returns_empty_string_on_request_error(mock_get):
...
def test_fetch_raw_content_returns_empty_string_on_location_parse_error():
    result = fetch_raw_content("///////example.net/data.csv")
    assert result == ""


@patch("datagovuk.directory.preview_utils.requests.get")
def test_fetch_raw_content_falls_back_to_iso8859_on_utf8_decode_error(mock_get):
    mock_response = MagicMock()
...
@patch("datagovuk.directory.preview_utils.requests.get")
def test_fetch_raw_content_request_has_range_header(mock_get):
    mock_response = MagicMock()
...
def test_fetch_csv_returns_parsed_rows(mock_fetch):
...
@patch("datagovuk.directory.preview_utils.fetch_raw_content")
def test_fetch_csv__returns_empty_list_when_no_content(mock_fetch):
...
@patch("datagovuk.directory.preview_utils.fetch_raw_content")
def test_fetch_csv__returns_empty_list_on_csv_error(mock_fetch, mock_reader):
...
@patch("datagovuk.directory.preview_utils.fetch_raw_content")
def test_fetch_csv__skips_empty_rows(mock_fetch):
    mock_fetch.return_value = "name,age\n\nJohn,30"
