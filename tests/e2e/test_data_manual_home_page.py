...
def test_data_manual_homepage_heading(page, live_server_url):
...
def test_data_manual_homepage_links(page, live_server_url):
    page.goto(live_server_url + reverse("data_manual:home"))
    for name, href in DATA_MANUAL_PAGES.items():
        link = page.get_by_role("link", name=name, exact=True)
        expect(link).to_have_attribute("href", href)


def test_feedback_link(page, live_server_url):
    page.goto(live_server_url + reverse("data_manual:home"))
...
def test_tell_us_what_you_think_link(page, live_server_url):
    page.goto(live_server_url + reverse("data_manual:home"))
...
def test_join_our_community_link(page, live_server_url):
    page.goto(live_server_url + reverse("data_manual:home"))
