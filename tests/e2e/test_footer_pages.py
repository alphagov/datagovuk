...
def test_about_page_content(page, live_server_url):
    ...
    expect(page.get_by_role("heading", name="Changes to the previous data.gov.uk functionality")).to_be_visible()
...
def test_accessibility_page_content(page, live_server_url):
    ...
    expect(page.get_by_role("heading", name="How accessible this website is")).to_be_visible()
    expect(page.get_by_role("heading", name="Feedback and contact information")).to_be_visible()
...
def test_support_page_content(page, live_server_url):
    ...
    expect(page.get_by_role("heading", name="If you’re a civil servant")).to_be_visible()  # noqa: RUF001
    expect(page.get_by_role("heading", name="Other requests")).to_be_visible()
...
def test_privacy_and_terms_page_content(page, live_server_url):
    ...
    expect(page.get_by_role("heading", name="Terms of use", exact=True)).to_be_visible()
    expect(page.get_by_role("heading", name="Who we are")).to_be_visible()
    expect(page.get_by_role("heading", name="Responsibility for datasets")).to_be_visible()
