...
def test_survey_banner_stays_hidden_after_dismiss_and_page_refresh(lazy_page, get_cookie, live_server_url) -> None:
    lazy_page.goto(live_server_url)
    lazy_page.get_by_role("button", name="Accept additional cookies").click()
    lazy_page.locator(".datagovuk-close").click()
    lazy_page.goto(live_server_url)
...
def test_survey_banner_dismiss_without_accepting_cookies(lazy_page, get_cookie, live_server_url) -> None:
    lazy_page.goto(live_server_url)
    expect(lazy_page.get_by_text("Help us improve the National Data Library")).to_be_visible()
    lazy_page.locator(".datagovuk-close").click()
    expect(lazy_page.get_by_text("Help us improve the National Data Library")).not_to_be_visible()
...
def test_no_cookie_for_survey_banner_when_all_cookies_rejected(lazy_page, get_cookie, live_server_url) -> None:
    lazy_page.goto(live_server_url)
